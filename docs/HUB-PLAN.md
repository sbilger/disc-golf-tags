...
- **Shared session (`suite.js`)** — ONE organizer state for the whole suite (date-gate removed
  2026-07-03): modules open read-only, 🔑 code once = organizer everywhere, admin account (Jeff) =
  organizer everywhere. Supabase Auth later fills the exact same shape.
